...
if (getlanguageforalert == "en") {
    ...
    listadditionaldocumentsByALL = "Additional Documents";
    listadditionaldocumentsByOGA = "Additional OGA Documents";
    listadditionaldocumentsByCustoms = "Additional Customs Documents ";
...
function renderDocType(docset)
{
  //  var filterDiv = document.getElementById("FilterDiv").innerHTML;
    ...
    if (bEnableItemAssociationStatus)
    {
        ...
        var htmlAssociateItems = '<input type="button" value="' + buttonvalue + '"  style= "width:100%" onclick= "AssociateItems();" />';
        ...
        if (bEnableDeletedRecordsStatus == "true") {
            ...
            if (document.getElementById("DeleteFlagTest").value == "ScanRequestUploadDocsDeleted") {
                var htmlEnabledeletebutton = '<input type="checkbox" value=' + checkboxtext + ' checked= "true" id= "deleteText" name= "chkMarks" runat= "server" onclick= "Fn_ViewDeletedRecords();" />';
            }
            else {
                var htmlEnabledeletebutton = '<input type="checkbox" value=' + checkboxtext + ' id="deleteText" name="chkMarks" runat="server" onclick= "Fn_ViewDeletedRecords();" />';

            }
            leftTable = '<table class="DOCTYPETABLE" style="height:100%">' +
                '<tr><td>' + selectalltypes + '</td></tr>' +
                '<tr><td style="height:100%; vertical-align:top;">' + docTypesHTML + '</td></tr>' +
                '<tr><td>' + htmlAssociateItems + '</td></tr>' +
                '<tr><td>' + htmlEnabledeletebutton + '' + checkboxtext + '</td ></tr > ' +
    ...
    else
    {
        if (bEnableDeletedRecordsStatus == "true")
        {
            ...
            //DeleteEnabledTable = '<table class="DOCTYPETABLE" style="height:100%">' +
            ...
            leftTable = '<table class="DOCTYPETABLE" style="height:100%">' +
                '<tr><td style="height:100%; vertical-align:top;">' + docTypesHTML + '</td></tr>' +
             //'<tr><td>' + htmlEnabledeletebutton + '' + checkboxtext + '</td ></tr > ' +
             //  // '<tr><td<div class="navbar-header"><button type="button" id="sidebarCollapse" class="navbar-btn">'+ 
               //' <span></span> <span></span> <span></span> </button>  </div > </td></tr>' +
                //<img width="10" height="10" class="navbar-btn inactive" id="sidebarCollapse" onclick="Toggle()" src="Images/RS_Rejected.png" syle="padding-right: 5px; padding-left: 5px;">
                ...
                //'<li> <a href="#">List All Additional Documents </a></li > ' +
                //'<li> <a href="#">List All Additional Documents By Customs  </a></li > ' +
                //'<li> <a href="#">List All Additional Documents By OGA  </a></li > ' +
                '  <li> <a href="#" onclick="Fn_ViewDeletedRecords(this)">' + checkboxtext + '</a> </li>' +
            '</ul></nav><div id="content"><nav class="navbar navbar-default"><div class="container-fluid"><div class="navbar-header"> <img width="10" height="10" class="navbar-btn inactive" id="sidebarCollapse" onclick="Toggle()" src="Images/iconFilter.png" >'+
...
function AssociateItems() {
  ...
  //  alert('Selected documents:\n' + selectedDocId);
   // if (c > selectedDocId.length && selectedDocId.length > 0)
     ...
     //   sPageURL = 'http://localhost:56770/ItemAssociation.aspx?Documentid=' + selectedDocId;
       sPageURL = 'http://10.10.26.226/ScanUploadDocument/ItemAssociation.aspx?Documentid=' + selectedDocId;
...
function renderDocNames(docTypeIdx) {
    ...
    for (var docNameIdx in docTypes.uploadedDocuments)
    {
        ...
        if (docTypes.uploadedDocuments[docNameIdx].AdditionalDocRequestNumber != 0 && AdditionalDocumentFlag == "yes")
        {
